...
- Legacy TMS9918 modes (Graphic I/II, Text, Multicolor) with TMS sprites —
  SG-1000 games (`.sg`) and F-16 Fighting Falcon
...
- Save states (F5/F7 in the GUI; `--state-save`/`--state-load` headless),
  deterministic across processes; hold Backspace to rewind time
  (`rewind_seconds` of history in sesame.cfg)
...
- CRT filter, on by default (single-pass shader: barrel, scanlines, shadow
...
./build/sesame game.sms --no-crt          # raw pixels (CRT filter is on by default)
./build/sesame game.sms --kiosk           # arcade-cabinet mode: exclusive
                                          # fullscreen, hidden cursor
```
